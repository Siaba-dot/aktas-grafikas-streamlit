...
def best_match(target: str, candidates: List[str], strict: bool = True) -> Optional[str]:
    """STRICT: tikslus atitikimas; jei strict=False – taikomas „fuzzy“ (startswith/substr)."""
    t = norm(target)
    # strict
    for c in candidates:
        if norm(c) == t:
...
def anchor_coords(ws: Worksheet, row: int, col: int) -> Tuple[int,int]:
    """Grąžina ankarą (viršutinį-kairį) sujungto langelio; jei nesujungtas — tą patį (row,col)."""
...
def clear_weekday_marks_row(ws: Worksheet, row: int, day_cols: Dict[int,int]) -> None:
    """Išvalo Pn–Pn langelius eilutei, rašydamas į ankarą (jei sujungta)."""
...
def write_X_to_weekday(ws: Worksheet, row: int, col: int, value: str = "X") -> None:
    """Rašo 'X' į Pn–Pn langelį (į ankarą, jei sujungta)."""
...
# ================= Core: ODS → X + period testo surinkimas =================
def mark_X_and_collect_period_texts(
    ...
    """
    Grąžina:
      touched          – kiek akto eilučių paliesta
    ...
    # Rašom X (tik toms, kur NĖRA override — Pn–Pn sujungimų neliečiam, jei yra tekstiniai periodai)
    for r, nm in row_names.items():
        if nm not in xmap:
            continue
        has_override = nm in period_override
        if clear_existing and not has_override:
            clear_weekday_marks_row(ws, r, day_cols)
        if not has_override:
            for wd in xmap[nm]:
                c = day_cols.get(wd)
                if c:
...
def apply_periodiskumas_mon_fri_with_override(
    ...
    for r in range(start_row, end_row + 1):
        ...
        # period override turi pirmenybę
...
def write_period_notes(
    ...
    Įrašo tekstą 'N kartai per mėn.':
      ...
      - 'monday_cell': į 'Pirmadienis' langelį tos eilutės (saugiai į ankarą).
    """
    start_row = HEADER_ROW_INDEX + 1
    end_row = find_end_row(ws, start_row)
    name_col = detect_name_col(ws, start_row, end_row, header_map)
    ...
    for r in range(start_row, end_row + 1):
        ...
        if mode == "comment":
            ...
            write_X_to_weekday(ws, r, monday_col, txt)  # saugiai į ankarą
            count += 1
...
if st.button("🔄 ODS → X (Pn–Pn) → Periodiškumas → Kaina", type="primary", use_container_width=True):
    ...
    try:
        ...
        # X + periodiškumo tekstai iš ODS bloko
        ...
        # Periodiškumas (pirma override, kitaip – X Pn–Pn, minus šventės)
