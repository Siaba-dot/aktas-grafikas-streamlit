...
# - Grafiko žymėjimai (X) perkelti į AKTO lapo bloką C10:G50 (be atskiro lapo)
# - Periodiškumas/Kaina/SUMA — tik Excel formulės
# - A6/C7 — formulės iš Skaičiavimai!A2/B2
# - A8="Savaitės dienos", C8..G8 = Pir..Pen; B7 tuščias; K51 — bendra suma
# - PVM netaikomas
...
MARK_BLOCK_WIDTH = 5            # C..G
MARK_BLOCK_HEIGHT = 41          # 10..50
...
if schedule_file and act_file:
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Skaičiavimai: Pir..Pen (365 formulės), remiasi į A2/B2
            ...
            # Paimame pirmus 5 pasirinktos grafiko lentelės stulpelius ir supilame "X"/tuščia į C10:G50
            mark_cols = selected_cols[:MARK_BLOCK_WIDTH] if selected_cols else []
            # Jei vartotojas pasirinko <5 stulpelių, supilame tiek, kiek yra; likusius paliekame tuščius
            from openpyxl.utils import column_index_from_string
            top_col_letter, top_row = MARK_BLOCK_TOP_LEFT
            top_col_index = column_index_from_string(top_col_letter)  # C -> 3
            # Aukštis ribojamas iki 41 eilučių (10..50)
            ...
            for ci, col_name in enumerate(mark_cols):
                ...
                for r_off in range(max_rows_to_fill):
                    src_val = normalize_text(sched_df.iloc[r_off][col_name]) if col_name in sched_df.columns else ""
                    val = "X" if src_val and ("x" in src_val.lower()) else ""  # supaprastinta: jeigu yra 'x' — X
                    ws_act.cell(row=top_row + r_off, column=dest_col, value=val)

            #  ===== PERIODIŠKUMO FORMULĖS (nuo 9-os), remiasi į C10:G50 + Skaičiavimai!C2..G2 =====
            headers_act = list(act_df.columns)
            def find_col_exact_or_prefix(headers_list, target):
                for i, h in enumerate(headers_list):
            ...
            # mapping: eilutė r (9..), žymėjimų eilutė = r + 1 (nes blokas prasideda 10)
            letters_map = ["C","D","E","F","G"]  # Skaičiavimai!C2..G2 (Pir..Pen)
            for r in range(first_row, last_row + 1):
                ...
                # C10..G50: Monday..Friday X; Skaičiavimai!C2..G2: atitinkamos darbo dienos mėnesyje
                mark_row = r + 1  # 9 -> 10, 10 -> 11, ...
                parts = []
                ...
                # KAINA = Plotas * Įkainis * Periodiškumas (be ROUND; vizualiai 0.00)
        ...
        st.success('✅ Paruošta: žymėjimai perkelti į AKTĄ (C10:G50), formulės perskaičiuoja Periodiškumą/Kainą, bendra suma K51.')
