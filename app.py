...
# 2) Vieno mygtuko TESTAS su pavyzdžiais tame pačiame kataloge
st.subheader("2) 🧪 Vieno mygtuko testas su pavyzdiniais failais")
if st.button("🧪 Paleisti TESTĄ su: Ignalina_GRAFIKAS.xlsx + VPK_Ignalina_2026_01.xlsx", type="secondary", use_container_width=True):
    try:
        # Failų keliai (turi būti tame pačiame kataloge kaip ir app)
        demo_sched = Path("Ignalina_GRAFIKAS.xlsx")
        demo_act = Path("VPK_Ignalina_2026_01.xlsx")
        if not demo_sched.exists() or not demo_act.exists():
            st.error("Neradau pavyzdinių failų vietiniame kataloge: 'Ignalina_GRAFIKAS.xlsx' ir 'VPK_Ignalina_2026_01.xlsx'. Įkelk juos šalia programos ir bandyk dar kartą.")
        else:
            # Nuskaitom aktą
            wb = load_workbook(filename=str(demo_act), data_only=False)
            ws = wb.active
            # Nustatom datą į pasirinktas Metai/Mėnuo
            last_day = calendar.monthrange(int(target_year), int(target_month))[1]
            fmt_map = {"MM/DD/YYYY":"%m/%d/%Y","YYYY-MM-DD":"%Y-%m-%d","DD.MM.YYYY":"%d.%m.%Y","YYYY.MM.DD":"%Y.%m.%d"}
            ws["A6"].value = date(int(target_year), int(target_month), last_day).strftime(fmt_map.get(date_format, "%m/%d/%Y"))
            ws["C7"].value = f"{LT_MONTH_GENITIVE[int(target_month)]} 1-{last_day}"
            # Išvalom esamas žymas
            header_map = build_header_map(ws, HEADER_ROW_INDEX)
            start = HEADER_ROW_INDEX + 1
            end = find_end_row(ws, start)
            _ = clear_all_weekday_marks(ws, header_map, start, end)
            # Nuskaitom grafiką iš xlsx
            sched_items = read_schedule_xlsx(str(demo_sched), int(target_month))
            # Taikom
            updated, x_written, overrides_set, wd_counts = apply_schedule_to_act(
                wb=wb, year=int(target_year), month=int(target_month),
                sched_items=sched_items,
            ...
            # Išsaugom į atskirą failą
            out_name = f"TEST_Aktas_atnaujintas_{int(target_year):04d}-{int(target_month):02d}.xlsx"
        ...
        st.exception(e)
        st.error("TESTO metu įvyko klaida. Patikrink, ar pavyzdiniai failai yra šalia programos ir ar pasirinktas mėnuo yra grafike.")
