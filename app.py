...
# periodiškumą SKAIČIUOJANT PAGAL KALENDORIŲ (ne kopijuojant),
# ir Excel formulių įrašymu (Periodiškumas + Kaina + SUM viršuje).
#
# Kas daroma:
# - C7: "{MĖNESIO KILM.} 1-{paskutinė}" (pvz., "BALANDŽIO 1-30")
# - A6: paskutinė mėnesio data (YYYY-MM-DD)
# - B7: tuščia; 7-oje eilutėje SUM(Plotas), SUM(Periodiškumas), SUM(Kaina)
# - B8: fiksuotas "Plotas kv m./kiekis/val"
# - Periodiškumas (eilutės): IF("X", Skaičiavimai!<PIR..SEK sum>, 0) pagal kalendorių
# - Kaina: Plotas × Įkainis × Periodiškumas; formatas "0.00" tik rezultatui, neapvalinant tarpinių
# - PVM neskaičiuojamas
...
<div class="hero">
  ...
  <p><strong>Periodiškumą skaičiuojame pagal kalendorių</strong>, o ne perkopijuojame.</p>
</div>
""", unsafe_allow_html=True)
...
# RAW grafiko nuskaitymas (peržiūrai; periodiškumo NEkeliame iš grafiko)
...
# Mėnesio aliasai (nebenaudojami periodiškumo kopijavimui; paliekam peržiūrai)
LT_MONTH_ALIASES = {
...
# ——— SVARBU: NEBEKOPIJUOJAME PERIODIŠKUMO IŠ GRAFIKO ———
def update_act_from_schedule(act_df: pd.DataFrame, schedule_df: pd.DataFrame, month_cols: list):
    """Periodiškumo neliečiam — jis bus paskaičiuotas formulėmis pagal kalendorių."""
    return act_df.copy()
...
def lt_fixed_holidays(year: int) -> set:
    """Fiksuotos šventinės: 01-01, 02-16, 03-11, 05-01, 06-24, 07-06, 08-15, 11-01, 11-02, 12-24, 12-25, 12-26"""
    return {
        date(year, 1, 1),
        date(year, 2, 16),
        date(year, 3, 11),
...
def lt_variable_holidays(year: int,
    ...
    include_pentecost_sunday=False,   # Sekminės – pagal pageidavimą nešventinė
...
# Savaitės dienų aliasai (LT/EN/sutrump.)
WEEKDAY_ALIAS_MAP = {
...
with tab2:
    ...
    st.markdown("### Pasirinkite mėnesį ir metus")
    ...
    month_choice = st.selectbox("Mėnuo", options=months_in_block, index=default_idx,
                                 format_func=lambda m: MONTH_NAME_LT[m])

    month_cell_addr = st.text_input("Įrašyti mėnesį į langelį (A1 adresas)", value="C7")
    a1 = st.text_input("A1 diapazonas (peržiūrai, neprivaloma)", value=a1_default)
...
with tab3:
    st.markdown("### Šventinės dienos (Periodiškumo skaičiavimui)")
    include_easter_sunday = st.checkbox("Įtraukti Velykų sekmadienį", value=True)
    include_easter_monday = st.checkbox("Įtraukti Velykų pirmadienį", value=True)
    include_pentecost_sun  = st.checkbox("Įtraukti Sekminių sekmadienį", value=False)  # pagal pageidavimą: NE
    ...
    st.caption("Papildomos šventinės (rankiniu būdu): YYYY-MM-DD per kablelį.")
    ...
    # Laikinai saugom į session_state, kad pasinaudotume Tab4
    st.session_state['include_easter_sunday'] = include_easter_sunday
    st.session_state['include_easter_monday'] = include_easter_monday
    st.session_state['include_pentecost_sun'] = include_pentecost_sun
    st.session_state['include_mothers_day'] = include_mothers_day
    st.session_state['include_fathers_day'] = include_fathers_day
    st.session_state['extra_add_set'] = extra_add_set
    st.session_state['extra_del_set'] = extra_del_set

    # Peržiūrai: fiksuotos šventinės
    st.write("Fiksuotos šventinės:", sorted(list(lt_fixed_holidays(year_choice))))
...
with tab4:
    ...
    # NEKEIČIAME Periodiškumo – jį paskaičiuosime formulėmis
    updated_df = update_act_from_schedule(act_df, raw_sched_df, [])
    ...
    with st.spinner("Kuriame formules ir generuojame Excel..."):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Rašome lentelę (po to – įrašome formules į ws)
            updated_df.to_excel(writer, index=False, sheet_name=target_sheet_name)
            ws = writer.sheets[target_sheet_name]

            # ---- KALENDORIAUS DIENOS (Pir..Sek) – be kopijavimo iš grafiko ----
            fixed_h = lt_fixed_holidays(year_choice)
            variable_h = lt_variable_holidays(
                year_choice,
                include_easter_sunday=st.session_state.get('include_easter_sunday', True),
                include_easter_monday=st.session_state.get('include_easter_monday', True),
                include_pentecost_sunday=st.session_state.get('include_pentecost_sun', False),  # default False
                include_mothers_day=st.session_state.get('include_mothers_day', True),
                include_fathers_day=st.session_state.get('include_fathers_day', True),
            )
            extra_add_set = st.session_state.get('extra_add_set', set())
            extra_del_set = st.session_state.get('extra_del_set', set())

            holidays_final = (fixed_h.union(variable_h).union(extra_add_set)).difference(extra_del_set)
            # Saugiklis: Sekminės — išmetame
            es = easter_sunday(year_choice)
            holidays_final.discard(es + timedelta(days=49))

            weekday_counts = {w: 0 for w in range(7)}  # 0=Pir ... 6=Sek
            for d in calendar.Calendar().itermonthdates(year_choice, month_choice):
                if d.month == month_choice and d not in holidays_final:
                    weekday_counts[d.weekday()] += 1

            # ---- Skaičiavimai lapas (C2:I2) ----
            wb = writer.book
            ...
            # Priverstinis perskaičiavimas atidarant
            try:
                wb.calculation.fullCalcOnLoad = True
            except Exception:
                pass

            # ---- Metaduomenys (C7, A6, B7, B8) ----
            ...
            qty_col    = next((c for c in act_cols if any(k in _norm(c) for k in ['plotas','kiekis','kv','val'])), None)
            qty_idx    = act_cols.index(qty_col)  if qty_col  else None
            rate_idx   = act_cols.index(rate_col) if rate_col else None

            # ---- Periodiškumo formulės pagal "X" savaitės dienų stulpeliuose ----
            act_weekday_map = map_headers_to_weekdaysets(act_cols)

            def weekday_count_cell(w: int) -> str:
                return f"Skaičiavimai!{chr(67 + w)}2"  # C2..I2

            header_count_expr = {}
            for h, wset in act_weekday_map.items():
                header_count_expr[h] = "+".join(weekday_count_cell(w) for w in sorted(wset)) if wset else "0"

            n_rows = len(updated_df)

            # ---- 7 eil. SUM formulės ----
            last_data_row = n_rows + 1  # 1 – antraštės
            if qty_idx is not None:
                c = f"{excel_col_letter(qty_idx)}7"
                ws[c] = f"=SUM({excel_col_letter(qty_idx)}2:{excel_col_letter(qty_idx)}{last_data_row})"
                ws[c].number_format = '0.00'
            c = f"{excel_col_letter(period_idx)}7"
            ws[c] = f"=SUM({excel_col_letter(period_idx)}2:{excel_col_letter(period_idx)}{last_data_row})"
            ws[c].number_format = '0.00'
            c = f"{excel_col_letter(price_idx)}7"
            ws[c] = f"=SUM({excel_col_letter(price_idx)}2:{excel_col_letter(price_idx)}{last_data_row})"
            ws[c].number_format = '0.00'

            # ---- Eilučių formulės (Periodiškumas + Kaina) ----
            for i in range(2, n_rows + 2):
                # Periodiškumas = Σ IF(cell == "X", Skaičiavimai!..., 0)
                parts = []
                ...
                ws[f"{excel_col_letter(period_idx)}{i}"] = "=" + ("+".join(parts) if parts else "0")

                # Kaina = Plotas * Įkainis * Periodiškumas
    ...
    st.toast("✅ Periodiškumas pagal kalendorių ir formulės įrašytos.", icon="🎉")
