...
# Periodiškumą SKAIČIUOJANT PAGAL KALENDORIŲ, o ne kopijuojant iš kito Excel.
# Funkcijos:
# - C7: "{MĖNESIO KILM.} 1-{paskutinė}" (pvz., "BALANDŽIO 1-30")
# - A6: paskutinė mėnesio data (YYYY-MM-DD)
# - B7: tuščia; 7-oje eilutėje įrašomos SUM(Plotas), SUM(Periodiškumas), SUM(Kaina)
# - B8: fiksuotas "Plotas kv m./kiekis/val"
# - Skaičiavimai: Pir..Sek skaičiai (atsižvelgiant į šventines; Sekminės neįtraukiamos)
# - Periodiškumas: FORMULĖMIS pagal "X" stulpeliuose ir Skaičiavimai!Pir..Sek
# - Kaina: Plotas * Įkainis * Periodiškumas (formatas 0.00 tik rezultatui)
# - PVM neskaičiuojamas
...
<div class="hero">
  ...
  <p>Periodiškumą skaičiuojame <strong>pagal kalendorių</strong>, o ne perkopijuojame.</p>
</div>
""", unsafe_allow_html=True)
...
# RAW grafiko nuskaitymas (grafikas reikalingas tik peržiūrai/kaip kontekstas)
...
# Mėnesio aliasai grafiko paieškai (nebe naudojami periodiškumo kopijavimui)
LT_MONTH_ALIASES = {
...
def update_act_from_schedule(act_df: pd.DataFrame, schedule_df: pd.DataFrame, month_cols: list):
    """
    Paliekame aktą nekeičiant 'Periodiškumas' – jis bus paskaičiuotas FORMULĖMIS pagal kalendorių.
    Ši funkcija grąžina act_df tokį patį (galite išplėsti, jei reikės kitų laukų atnaujinimo).
    """
    return act_df.copy()
...
def lt_variable_holidays(year: int,
    ...
    include_pentecost_sunday=False,   # Sekminės – neskaičiuojame kaip šventinės (pagal tavo pageidavimą)
...
with tab3:
    st.markdown("### Šventinės dienos ir kalendoriaus skaičiavimas (Periodiškumui)")
    ...
    include_mothers_day    = st.checkbox("Įtraukti Motinos dieną", value=True)
    include_fathers_day    = st.checkbox("Įtraukti Tėvo dieną", value=True)

    variable_h = lt_variable_holidays(year_choice,
                                      include_easter_sunday, include_easter_monday,
                                      include_pentecost_sun, include_mothers_day, include_fathers_day)
    ...
    # Saugiklis: Sekminės laikomos "darbo diena"
    es = easter_sunday(year_choice)
    ps = es + timedelta(days=49)
    ...
    with col_b:
        wd_union = sum(weekday_counts[w] for w in range(0,5))
...
with tab4:
    ...
    updated_df = update_act_from_schedule(act_df, raw_sched_df, [])  # month_cols nenaudojami

    # Įkainio konvertavimas į skaičių (be apvalinimų)
    ...
    with st.spinner("Kuriame formules ir generuojame Excel..."):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Akto antraščių atpažinimas kaip savaitės dienų stulpeliai (X žymoms)
            ...
            # ——— 7-os eilutės SUM ———
            try:
                ws["B7"] = ""  # kaip prašei – tuščia
                last_data_row = n_rows + 1  # 1 – header
                if qty_idx is not None:
                    cell = f"{excel_col_letter(qty_idx)}7"
                    ws[cell] = f"=SUM({excel_col_letter(qty_idx)}2:{excel_col_letter(qty_idx)}{last_data_row})"
                    ws[cell].number_format = '0.00'
                if period_idx is not None:
                    cell = f"{excel_col_letter(period_idx)}7"
                    ws[cell] = f"=SUM({excel_col_letter(period_idx)}2:{excel_col_letter(period_idx)}{last_data_row})"
                    ws[cell].number_format = '0.00'
                if price_idx is not None:
                    cell = f"{excel_col_letter(price_idx)}7"
                    ws[cell] = f"=SUM({excel_col_letter(price_idx)}2:{excel_col_letter(price_idx)}{last_data_row})"
                    ws[cell].number_format = '0.00'
            except Exception as e:
                st.warning(f"Nepavyko įrašyti SUM į 7 eilutę: {e}")

            # ——— Eilučių formulės ———
            for i in range(2, n_rows + 2):
                # Periodiškumas = Σ IF(header_cell == "X", Skaičiavimai!..., 0)
                parts = []
