...
# - 1–7 eil. paliekamos kaip originale; antraštės prasideda 8-oje; duomenys nuo 9-os
# - Periodiškumas/Kaina/SUMA rašomi tik kaip Excel formulės (be Python galutinių skaičiavimų)
# - Grafiko įkėlimas (.ods/.xlsx/.xls), A1 diapazono pasirinkimas, header wizard, filtrai palikti
# - A6: paskutinė mėnesio diena (formulė), C7: mėnesio tekstas "SAUSIO 1-31" (formulė)
# - B7: tuščia; A8: tuščia; C8: nevaloma (lieka "Pirmadienis")
# - PVM netaikomas
...
# ========================= KONSTANTOS =========================
HEADER_TOP_ROW = 7              # 7 eilė – viršutinė sujungtų antraščių juosta / viršutiniai akto įrašai
HEADER_BOTTOM_ROW = 8           # 8 eilė – lentelės antraštės
DATA_START_ROW = HEADER_BOTTOM_ROW + 1  # 9 eilė – duomenys (formulės taikomos nuo šios eil.)
...
# ========================= Akto nuskaitymas (raw + suformuotos antraštės) =========================
...
if schedule_file and act_file:
    try:
        # ==== GRAFIKAS (peržiūra, A1 diapazonas, header wizard, filtrai) ====
        ...
        # ==== AKTAS: nuskaitymas su 7–8 eil. antraštėmis; 1–7 eil. paliekamos ====
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # 2) Aktas — rašome taip, kad antraštės būtų 8-oje, duomenys nuo 9-os
            act_df.to_excel(writer, index=False, sheet_name=act_sheet, startrow=HEADER_BOTTOM_ROW-1)
            wb = writer.book
            ws_act = wb[act_sheet]

            # 2a) Grąžiname VIRŠŲ (1–7 eilutės) lygiai taip, kaip buvo originale (reikšmes)
            ...
            # --- Viršaus formulės (susietos su Skaičiavimai!A2/B2) ---
            date_cell_addr  = "A6"  # paskutinė pasirinkto mėnesio diena
            month_cell_addr = "C7"  # mėnesio tekstas "SAUSIO 1-31"
            ...
            # A6: paskutinė diena (formulė); C7: mėnesio tekstas (formulė)
            ws_act[date_cell_addr] = "=EOMONTH(DATE(Skaičiavimai!$A$2,Skaičiavimai!$B$2,1),0)"
            ws_act[date_cell_addr].number_format = "yyyy-mm-dd"
            ws_act[month_cell_addr] = (
            ...
            # Tikslūs išvalymai viršuje/antraštėse:
            ws_act["B7"] = None   # B7 – tuščia (antraštės nuo 8-os)
            # A8 – užtikrinam, kad nebeliktų "Mėnuo"/"Savaitės dienos"
            val_A8 = ws_act["A8"].value
            if isinstance(val_A8, str) and val_A8.strip().lower() in ("mėnuo", "menuo", "savaitės dienos", "savaites dienos"):
                ws_act["A8"] = None
            else:
                ws_act["A8"] = None
            # !!! C8 NELIEČIAM — ten turi likti "Pirmadienis"
            ...
            # 5) GALUTINĖ SUMA — po lentelės K stulpelyje (ir "Kaina" stulpelio apačioje)
            sum_row = last_row + 1
            sum_formula = f"=SUM({addr(col_kaina, first_row)}:{addr(col_kaina, last_row)})"

            # Įrašome į K stulpelį (kaip prašei)
            ws_act[f"K{sum_row}"] = sum_formula
            ws_act[f"K{sum_row}"].number_format = "0.00"

            # Taip pat į "Kaina" stulpelio apačią (jei jis ne K)
            ws_act[addr(col_kaina, sum_row)] = sum_formula
        ...
        st.success('✅ Paruošta: 1–7 eil. paliktos, antraštės 8-oje, formulės nuo 9-os, suma K stulpelyje po lentele.')
