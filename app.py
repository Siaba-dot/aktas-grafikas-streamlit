...
with tab2:
    ...
    month_cell_addr = st.text_input("Mėnesio tekstas į langelį (A1 adresas)", value="C7")
...
with tab3:
    ...
    # Tik peržiūrai: fiksuotos šventinės
    fixed_preview = lt_fixed_holidays(year_choice)
    st.write("Fiksuotos:", sorted(list(fixed_preview)))
...
with tab4:
    ...
    with st.spinner("Kuriame formules ir generuojame Excel..."):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # === KALENDORIAUS DIENŲ SKAIČIAI (Pir..Sek) — be kopijavimo iš grafiko ===
            fixed_h = lt_fixed_holidays(year_choice)
            variable_h = lt_variable_holidays(
                year_choice,
                include_easter_sunday=True,
                include_easter_monday=True,
                include_pentecost_sunday=False,  # Sekminės laikomos "darbo diena"
                include_mothers_day=True,
                include_fathers_day=True,
            )
            # UI papildomos šventinės (jei buvo suvestos)
            # (paimame iš tab3 valdiklių; jei reikia, persikelkite kintamuosius į session_state)
            extra_add_set = parse_dates_csv(st.session_state.get('Pridėti datas', "") if 'Pridėti datas' in st.session_state else "")
            extra_del_set = parse_dates_csv(st.session_state.get('Pašalinti datas', "") if 'Pašalinti datas' in st.session_state else "")
            ...
            # Savaitės dienų skaičiai pagal kalendorių (0=Pir ... 6=Sek), atmetant šventines
            weekday_counts = {w: 0 for w in range(7)}
            cal = calendar.Calendar()
            for d in cal.itermonthdates(year_choice, month_choice):
                if d.month != month_choice:
                    continue
                if d not in holidays_final:
                    weekday_counts[d.weekday()] += 1

            # === "Skaičiavimai" lapas: PIR..SEK skaičiai (C2..I2) ===
            book = writer.book
            ...
            # === B8 fiksuotas; B7 tuščias ===
            ws['B8'] = "Plotas kv m./kiekis/val"
            ws['B7'] = ""

            # === Stulpelių indeksai: Periodiškumas / Kaina / Plotas / Įkainis ===
            act_cols = list(updated_df.columns)
            ...
            # === 7-oje eilutėje SUM(Plotas), SUM(Periodiškumas), SUM(Kaina) ===
            last_data_row = n_rows + 1  # 1 yra antraštės
            ...
            # (Pasirinktinai) apačioje – "Iš viso (be PVM)"
            total_row = n_rows + 3
    ...
    st.toast("✅ Periodiškumas apskaičiuotas pagal kalendorių ir formulės įrašytos.", icon="🎉")
