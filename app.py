...
# ir Excel formulėmis: Periodiškumas + Kaina (be PVM, 2 skaitmenys po kablelio tik rezultatui).
#
# Funkcijos:
...
# - Kaina: Plotas * Įkainis * Periodiškumas, rodoma 0.00, tarpinių nereformatuoja/neapvalina
...
<div class="hero">
  ...
  <p>Pasirinkite metus ir mėnesį → teisingos dienos, šventinės, <strong>Periodiškumas</strong> ir <strong>Kaina</strong> formulėmis.</p>
</div>
""", unsafe_allow_html=True)
...
# Akto atnaujinimas (periodiškumą vis tiek skaičiuosime formulėmis)
...
def lt_fixed_holidays(year: int) -> set:
    return {
        date(year, 1, 1), date(year, 2, 16), date(year, 3, 11),
        date(year, 5, 1), date(year, 6, 24), date(year, 7, 6),
        date(year, 8, 15), date(year, 11, 1),
        date(year, 12, 24), date(year, 12, 25), date(year, 12, 26),
    }

def lt_variable_holidays(year: int,
    include_easter_sunday=True, include_easter_monday=True,
    include_pentecost_sunday=True, include_mothers_day=True, include_fathers_day=True) -> set:
    es = easter_sunday(year); em = es + timedelta(days=1)
    ps = es + timedelta(days=49); md = first_sunday(year, 5); fd = first_sunday(year, 6)
    s = set()
...
# Savaitės dienų aliasai (platus: „ketvirtadie“, „penktadie“ ir pan.)
WEEKDAY_ALIAS_MAP = {
...
with tab3:
    ...
    with col_b:
        wd_union = sum(weekday_counts[w] for w in range(0,5))  # bendra darbo dienų suma (informacinė)
...
with tab4:
    ...
    # Įkainių konvertavimas į skaičius (be apvalinimo)
