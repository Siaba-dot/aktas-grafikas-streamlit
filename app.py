...
# Fiksuoti ODS blokai (A1 formatu) – kiekviename yra lygiai 5 stulpeliai (Pirm–Penkt)
...
# Griežtas mappingas ODS → akto pavadinimams (pildyk realiais tekstais)
NAME_OVERRIDE = {
...
def set_header_fields(ws: Worksheet, year: int, month: int, date_fmt: str = "MM/DD/YYYY"):
    """A6: paskutinė mėnesio diena; C7: mėnuo kilmininku + '1-XX'."""
...
def read_schedule_ods(ods_file) -> pd.DataFrame:
    """Skaitom ODS be antraščių, kad A1 koordinatės (pvz., 'C7:G47') atitiktų realias vietas."""
...
def best_match(target: str, candidates: List[str], strict: bool = True) -> Optional[str]:
    """STRICT: tikslus atitikimas; jei strict=False – „fuzzy“ (startswith/substr)."""
    t = norm(target)
    for c in candidates:
        if norm(c) == t:
            return c
...
# --- Merged cells helperiai ---
def find_merged_range(ws: Worksheet, row: int, col: int):
    """Grąžina CellRange, jei (row,col) priklauso sujungtam diapazonui; kitu atveju None."""
...
def anchor_coords(ws: Worksheet, row: int, col: int) -> Tuple[int,int]:
    """Grąžina inkarą (viršutinį–kairį) sujungto langelio; jei nesujungtas — tą patį (row,col)."""
    rng = find_merged_range(ws, row, col)
    if rng:
        return rng.min_row, rng.min_col
    return row, col

def clear_weekday_marks_row(ws: Worksheet, row: int, day_cols: Dict[int,int]) -> None:
    """Išvalo Pn–Pn langelius eilutei, rašydamas į inkarą (jei sujungta)."""
    for _, c in day_cols.items():
        ar, ac = anchor_coords(ws, row, c)
        ws.cell(ar, ac).value = None

def write_X_to_weekday(ws: Worksheet, row: int, col: int, value: str = "X") -> None:
    """Rašo 'X' į Pn–Pn langelį (į inkarą, jei sujungta)."""
    ar, ac = anchor_coords(ws, row, col)
    ws.cell(ar, ac).value = value

# --- Akto eilučių ištraukimas ---
...
def mark_X_and_collect_period_texts(
    ...
    """
    Grąžina:
      touched          – kiek akto eilučių paliesta (kurioms priskirtas bent kažkoks signalas)
      xmap             – {akto_eilutė -> {0..4}} pažymėti Mon–Fri indeksai
      period_override  – {akto_eilutė -> N} (jei rasta 'N kartas/kartai...' arba skaičius)
      period_text      – {akto_eilutė -> 'N kartai per mėn.'} (pilnas tekstas)
    ...
    name_cols_candidates = [0, 1]  # paslaugų pavadinimo kolonų kandidatės visame ODS
    ...
    # Eilutė po eilutės per bloką
    for abs_row in range(r0, r1 + 1):
        rel_row = abs_row - r0
        row_slice = sub.iloc[rel_row, :]

        # Paslaugos pavadinimas (iš A/B ar ilgiausio teksto eilutėje)
        svc_text = None
        ...
        # Periodiškumo tekstas / skaičius — ieškom 5 stulpeliuose
        override_val = None
        override_txt = None
        for cell_val in row_slice.tolist():
            if isinstance(cell_val, str):
                s = cell_val.strip().lower()
                m = re.search(r"(\d+)\s*kart", s)  # '1 kartas', '2 kartai', ...
                if m:
        ...
        # Jei nėra nei X, nei override — skip
    ...
    # Tik pažymime, kad šioms eilutėms bus kažkas daroma (touched skaičiui)
    touched = len(set(list(xmap.keys()) + list(period_override.keys())))
    return touched, xmap, period_override, period_text, report

# ================= Globalus Pn–Pn valymas =================
def clear_all_weekday_marks(ws: Worksheet, header_map: dict) -> int:
    """Išvalo VISUS Pirm–Penkt langelius visoje pagrindinėje lentelėje (saugiai per inkarą)."""
    day_cols = {}
...
# ================= Periodiškumas (override → X) + X rašymas =================
def apply_periodiskumas_and_write_x(
    wb: Workbook,
    year: int,
    month: int,
    xmap: Dict[str, set],
    ...
    """
    1) Jei yra period_override[row_name] – periodiškumas = override (tiesiog skaičius), X nerašomi.
    2) Jei nėra override – X parašomi (saugiai) ir periodiškumas skaičiuojamas iš Mon–Fri skaičių (minus šventės).
    Grąžina: (kiek eilučių atnaujinta, kiek X parašyta, Mon–Fri dienų skaičiai)
    """
    ws = wb.active
    ...
    for r in range(start_row, end_row + 1):
        ...
        # 1) Override turi pirmenybę – X nerašom
        if row_name in period_override:
            ws.cell(r, period_col).value = int(period_override[row_name])
            updated += 1
            continue

        # 2) Jei yra X rinkinys – parašom X ir skaičiuojame periodą
    ...
    return updated, x_written, wd_counts

# ================= Period teksto įrašymas =================
def write_period_notes(
    ...
    Įrašo tekstą 'N kartai per mėn.':
      - 'comment': kaip komentarą prie 'Periodiškumas' langelio,
      - 'monday_cell': į 'Pirmadienis' langelį tos eilutės (saugiai į inkarą).
    """
    start_row = HEADER_ROW_INDEX + 1
    end_row = find_end_row(ws, start_row)
    name_col = detect_name_col(ws, start_row, end_row, header_map)
    ...
    for r in range(start_row, end_row + 1):
        ...
        if mode == "comment":
            ws.cell(r, period_col).comment = Comment(f"{txt}", "SI-Copilot")
            count += 1
        elif mode == "monday_cell" and monday_col:
            write_X_to_weekday(ws, r, monday_col, txt)
            count += 1
...
def write_kaina_formulas(ws: Worksheet, header_map: Dict[str,int], replace_existing: bool = True) -> int:
    """
    Įrašo 'Kaina' formules: =TRUNC(Plotas*Įkainis*Periodiškumas, 2).
    Skaičių formatas – 0.00
    """
...
with st.sidebar:
    ...
    clear_existing_x = st.checkbox("Išvalyti SENAS Pn–Pn žymas (rekomenduojama)", value=True)
...
if st.button("🔄 ODS → X (Pn–Pn) → Periodiškumas → Kaina", type="primary", use_container_width=True):
    ...
    try:
        ...
        # 1) VISŲ Pn–Pn langelių globalus valymas (saugiai, su inkarais)
        if clear_existing_x:
            with st.spinner("Valau senas Pn–Pn žymas..."):
                _cleared = clear_all_weekday_marks(ws, header_map)

        # 2) Iš ODS: surenkam X + periodiškumo tekstus/skaičius
        ...
        # 3) Periodiškumas + naujų X rašymas (override → X)
        with st.spinner("Skaičiuoju „Periodiškumą“ ir rašau naujus X..."):
            updated, x_written, wd_counts = apply_periodiskumas_and_write_x(
                wb=wb,
        ...
        # 4) Periodiškumo tekstų įrašymas
