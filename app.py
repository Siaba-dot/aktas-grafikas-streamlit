...
def read_schedule_ods(ods_file, month: int) -> List[Dict]:
    """
    ODS grafiko skaitymas:
    - Pn–Pn bloką imame pagal ODS_GRID_RANGES[mėnuo] (A1 koordinatės).
    - Pavadinimų stulpelį aptinkame KAIRĖJE nuo Pn–Pn bloko, ieškodami labiausiai „tekstinio“.
    - Grąžiname elementus su: name, section (aukštas), wdset (X savaitės dienos), override (N kart./mėn.)
    """
    try:
        df = pd.read_excel(ods_file, engine="odf", header=None)
    except Exception:
        st.error("Nepavyko atidaryti ODS (reikia `odfpy` paketo) arba failas sugadintas.")
        raise
    ...
    # Surandam pavadinimų stulpelį kairėje nuo bloko
    name_col = None
    best_score = -1
    for cc in range(c0 - 1, -1, -1):
        ...
        if score > best_score:
            ...
            name_col = cc
    if name_col is None:
        name_col = 0  # kraštutinis fallback
    ...
    for rr in range(r0, r1 + 1):
        ...
        # paieška aplink pavadinimą (kartais „2 kartai per mėn.“ būna šalia pavadinimo)
