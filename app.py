...
# "header wizard", mėnesių blokais, darbo dienomis ir .xlsx generavimu su EXCEL FORMULĖMIS.
...
# Žymos (tekstas) sugeneravimas iš pasirinktų kol.
...
# ========= Kintamos šventės ir darbo dienos =========
...
if schedule_file and act_file:
    try:
        ...
        # ==== ŠVENTINĖS ir DARBO DIENOS (informacinė dalis) ====
        ...
        # ====== SUGENERUOJAM „Žyma“ grafike, reikalingą XLOOKUP ======
        sched_task_col = sched_df.columns[0]  # prielaida: 1-as stulpelis – užduotis/patalpa
        marker_col = "Žyma"
        if marker_col not in sched_df.columns:
            sched_df[marker_col] = sched_df.apply(lambda r: collect_marker_from_columns(r, selected_cols), axis=1)

        # ====== Eksportas: DVI LENTELĖS + FORMULĖS AKTE ======
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            # 1) Grafikas (su Žyma)
            sched_df.to_excel(writer, index=False, sheet_name='Grafikas')
            # 2) Aktas – pirma rašome duomenis, po to perrašome skaičiuojamus langelius formulėmis
            act_sheet_name = act_sheet if act_sheet else 'Akto_lapas'
            act_df.to_excel(writer, index=False, sheet_name=act_sheet_name)

            wb = writer.book
            ws_act = wb[act_sheet_name]
            ws_sched = wb['Grafikas']

            # Akto stulpelių paieška pagal pavadinimų fragmentus (nieko nepervadinam)
            act_cols = list(act_df.columns)

            def find_col(parts):
                for c in act_cols:
                    nc = normalize_text(c).lower()
                    if all(p in nc for p in parts):
                        return c
                return None

            name_col = act_cols[0]  # 1-as stulpelis – pavadinimas/patalpa/užduotis
            period_col = find_col(['periodi'])        # "Periodiškumas"
            qty_col    = find_col(['plotas']) or find_col(['kiekis'])
            rate_col   = find_col(['įkainis']) or find_col(['ikainis'])
            price_col  = find_col(['kaina'])

            from openpyxl.utils import get_column_letter
            def col_idx(col_name): return act_cols.index(col_name) + 1
            def addr(ci, ri): return f"{get_column_letter(ci)}{ri}"

            name_idx   = col_idx(name_col)
            period_idx = col_idx(period_col) if period_col else None
            qty_idx    = col_idx(qty_col) if qty_col else None
            rate_idx   = col_idx(rate_col) if rate_col else None
            price_idx  = col_idx(price_col) if price_col else None

            # Grafiko stulpelių indeksai (Užduotis + Žyma)
            sched_cols = list(sched_df.columns)
            sched_task_col_idx   = sched_cols.index(sched_task_col) + 1
            sched_marker_col_idx = sched_cols.index(marker_col) + 1
            sched_last_row = ws_sched.max_row

            # Įrašome FORMULES per visas eiles (2..N)
            n_rows = ws_act.max_row
            for r in range(2, n_rows + 1):
                task_ref = addr(name_idx, r)

                # PERIODIŠKUMAS = XLOOKUP(Užduotis, Grafikas!Užduotis, Grafikas!Žyma, "")
                if period_idx:
                    period_ref = addr(period_idx, r)
                    ws_act[period_ref].value = (
                        f'=XLOOKUP({task_ref}, '
                        f'Grafikas!{get_column_letter(sched_task_col_idx)}2:'
                        f'{get_column_letter(sched_task_col_idx)}{sched_last_row}, '
                        f'Grafikas!{get_column_letter(sched_marker_col_idx)}2:'
                        f'{get_column_letter(sched_marker_col_idx)}{sched_last_row}, "")'
                    )

                # KIEKIS = IFERROR(VALUE(Periodiškumas), "")  (jei žymoje skaičius)
                if qty_idx and period_idx:
                    qty_ref = addr(qty_idx, r)
                    period_ref = addr(period_idx, r)
                    ws_act[qty_ref].value = f'=IFERROR(VALUE({period_ref}), "")'

                # KAINA = Įkainis * Kiekis (be ROUND; rodymui 0.00)
                if price_idx and rate_idx and qty_idx:
                    price_ref = addr(price_idx, r)
                    rate_ref  = addr(rate_idx, r)
                    qty_ref   = addr(qty_idx, r)
                    ws_act[price_ref].value = f'={rate_ref}*{qty_ref}'
                    ws_act[price_ref].number_format = '0.00'  # rodymas 2 skaitmenys, neapvalinant skaičiavimo
        ...
        st.success('✅ Paruošta: galutinės reikšmės įrašytos kaip Excel FORMULĖS. Keičiant įvestį akte – rezultatai persiskaičiuoja automatiškai.')
        st.info('Pastaba: PVM netaikomas. Kainos rodymas 0.00 (be ROUND skaičiavime).')
